...
class EDD_Extension_Activation {
	...
	public function missing_edd_notice() {
		if ( $this->has_edd ) {
			...
			$link = '<a href="' . $url . '">' . __( 'activate it', 'edd-payout-receipts' ) . '</a>';
		} else {
			...
			$link = '<a href="' . $url . '">' . __( 'install it', 'edd-payout-receipts' ) . '</a>';
		}

		echo '<div class="error"><p>' . $this->plugin_name . sprintf( __( ' requires Easy Digital Downloads! Please %s to continue!', 'edd-payout-receipts' ), $link ) . '</p></div>';
	}
}
...
class EDD_Commissions_Activation {
	...
	/**
	 * Display notice if EDD Commissions isn't installed
	 *
	 * @access      public
	 * @since       1.0.0
	 * @return      string The notice to display
	...
	public function missing_commissions_notice() {
		if ( $this->has_edd_commissions ) {
			echo '<div class="error"><p>' . $this->plugin_name . sprintf( __( ' requires %sEDD Commissions%s. Please activate it to continue.', 'edd-payout-receipts' ), '<a href="https://easydigitaldownloads.com/downloads/commissions/" title="EDD Commissions" target="_blank">', '</a>' ) . '</p></div>';
		} else {
			echo '<div class="error"><p>' . $this->plugin_name . sprintf( __( ' requires %sEDD Commissions%s. Please install it to continue.', 'edd-payout-receipts' ), '<a href="https://easydigitaldownloads.com/downloads/commissions/" title="EDD Commissions" target="_blank">', '</a>' ) . '</p></div>';
		}
	}
}
